<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.states')"></Head>
        
        <template #header>
            {{ $trnsl('content.states') }}
        </template>
...
<script setup>
    ...
    const headers = [
                    ...
                    {name: 'country.name', type: 'text', text: trnsl('content.country'), align: 'left'},
                ];
    ...
    const fields = [
                    ...
                    {name: 'country_id', type: 'select', text: trnsl('content.country'), items:[], style: 'sm:col-span-4'},
                ];
    ...
    const deleteRecord = (row) => {
        ...
        alert.fire({
            ...
            if(result.isConfirmed){
                form.delete(route('settings.states.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const executeAction = ( params ) => {
        if (params[1]==='edit') {
            ...
            routePath.value = 'settings.states.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.states')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'settings.states.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.states')
            openModal()
        } else if (button == 'exit') {
            home()
        }
    ...
    onMounted(() => {
        fields[2].items = props.countries;
    }) 
