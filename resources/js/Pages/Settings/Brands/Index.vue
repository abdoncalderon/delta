<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.brands')"></Head>
        
        <template #header>
            {{ $trnsl('content.brands') }}
        </template>
...
<script setup>
    ...
    const deleteRecord = (row) => {
        ...
        alert.fire({
            ...
            if(result.isConfirmed){
                form.delete(route('settings.brands.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const executeAction = ( params ) => {
        if (params[1]==='edit') {
            ...
            routePath.value = 'settings.brands.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.brands')
    ...
    const clickIconToolBar = ( button ) => {
        if (button === 'new') {
            ...
            routePath.value = 'settings.brands.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.brands')
            openModal()
        } else if (button == 'exit') {
            home()
        }
