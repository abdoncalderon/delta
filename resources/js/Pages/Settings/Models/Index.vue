<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.models')"></Head>
        
        <template #header>
            {{ $trnsl('content.models') }}
        </template>
...
<script setup>
    ...
    const headers = [
        ...
        {name: 'brand.name', type: 'text', text: trnsl('content.brand'), align: 'left'},
    ];
    ...
    const fields = [
        ...
        {name: 'brand_id', type: 'select', text: trnsl('content.brand'), style: 'sm:col-span-4', items:[]},
    ];
    ...
    const deleteRecord = (row) => {
        ...
        dialog.fire().then((result) =>{
            if(result.isConfirmed){
                form.delete(route('settings.models.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const getAction = ( action ) => {
        if (action.name==='edit') {
            ...
            routePath.value = 'settings.models.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.models')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'settings.models.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.models')
            openModal()
        } else if (button == 'home') {
            home()
        }
