<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.unities')"></Head>
        
        <template #header>
            {{ $trnsl('content.unities') }}
        </template>
...
<script setup>
    ...
    const headers = [
        ...
        {name: 'code', type: 'text', text: trnsl('content.code'), align: 'left'},
    ];
    ...
    const fields = [
        {name: 'id', type: 'hidden'},
        {name: 'name', type: 'text', text: trnsl('content.name'), style: 'sm:col-span-2', length: 255 },
        {name: 'code', type: 'text', text: trnsl('content.code'), style: 'sm:col-span-2', length: 255 },
    ];
    ...
    const deleteRecord = (row) => {
        ...
        dialog.fire().then((result) =>{
            if(result.isConfirmed){
                form.delete(route('settings.unities.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const getAction = ( action ) => {
        if (action.name==='edit') {
            ...
            routePath.value = 'settings.unities.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.unities')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'settings.unities.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.unities')
            openModal()
        } else if (button == 'home') {
            home()
        }
