<template>
    ...
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.menus')"></Head>
...
<script setup>
    ...
    const headers = [
        ...
        {name: 'father', type: 'text', text: trnsl('content.father'), align: 'left'},
    ];
    ...
    const fields = [
        ...
        {name: 'text', type: 'text', text: trnsl('content.text'), style: 'sm:col-span-4', length: 255 },
        {name: 'route', type: 'text', text: trnsl('content.route'), style: 'sm:col-span-4', length: 255 },
        {name: 'icon', type: 'text', text: trnsl('content.icon'), style: 'sm:col-span-4', length: 255 },
        {name: 'father', type: 'select', text: trnsl('content.father'), style: 'sm:col-span-4', idField: 'father', items:[]},
        {name: 'isActive', type: 'switch', text: trnsl('content.active'), style: 'sm:col-span-1'},
    ];
    ...
    const deleteRecord = (row) => {
        ...
        dialog.fire().then((result) =>{
            if(result.isConfirmed){
                form.delete(route('settings.menus.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const getAction = ( action ) => {
        if (action.name==='edit') {
            ...
            routePath.value = 'settings.menus.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.menus')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'settings.menus.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.menus')
            openModal()
        } else if (button == 'home') {
            home()
        }
