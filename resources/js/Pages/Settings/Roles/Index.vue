<template>
    <AuthenticatedLayout>
        ...
        <template #header>
            {{ $trnsl('content.settings') }}
        </template>

        <template #content>
            <Head :title="$trnsl('content.roles')"></Head>
            <ToolBar
                :buttons="buttons"
...
<script setup>
    ...
    const deleteRecord = (row) => {
        ...
        alert.fire({
            ...
            if(result.isConfirmed){
                form.delete(route('settings.roles.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const executeAction = ( params ) => {
        if (params[1]==='edit') {
            ...
            routePath.value = 'settings.roles.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.roles')
    ...
    const clickIconToolBar = ( button ) => {
        if (button === 'new') {
            ...
            routePath.value = 'settings.roles.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.roles')
            openModal()
        } else if (button == 'exit') {
            home()
        }
