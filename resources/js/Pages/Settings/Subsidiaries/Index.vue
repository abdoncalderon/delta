<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.subsidiaries')"></Head>
        
        <template #header>
            {{ $trnsl('content.subsidiaries') }}
        </template>
        ...
        <template #content>
            ...
            <DataTable
                ...
                :table="trnsl('content.subsidiaries')"
                fieldSearch="name"
...
<script setup>
    ...
    const headers = [
                    ...
                    {name: 'company.name', type: 'text', text: trnsl('content.company'), align: 'left'},
                ];
    ...
    const fields = [
                    {name: 'id', type: 'hidden'},
                    {name: 'name', type: 'text', text: trnsl('content.name'), style: 'sm:col-span-3', lenght: 255 },
                    {name: 'code', type: 'text', text: trnsl('content.code'), style: 'sm:col-span-1', lenght: 6 },
                    {name: 'company_id', type: 'select', text: trnsl('content.company'), style: 'sm:col-span-2', items:[]},
                    {name: 'division_id', type: 'select', text: trnsl('content.division'), style: 'sm:col-span-2', items:[]},
                ];
    ...
    const deleteRecord = (row) => {
        ...
        alert.fire({
            ...
            if(result.isConfirmed){
                form.delete(route('settings.subsidiaries.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const executeAction = ( params ) => {
        if (params[1]==='edit') {
            ...
            routePath.value = 'settings.subsidiaries.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.subsidiaries')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'settings.subsidiaries.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.subsidiaries')
            openModal()
        } else if (button == 'exit') {
            home()
        }
    ...
    onMounted(() => {
        fields[3].items = props.companies;
        fields[4].items = props.divisions;
    }) 
