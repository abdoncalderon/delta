<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.divisions')"></Head>
        
        <template #header>
            {{ $trnsl('content.divisions') }}
        </template>
...
<script setup>
    ...
    const deleteRecord = (row) => {
        ...
        alert.fire({
            ...
            if(result.isConfirmed){
                form.delete(route('settings.divisions.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const executeAction = ( params ) => {
        if (params[1]==='edit') {
            ...
            routePath.value = 'settings.divisions.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.divisions')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'settings.divisions.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.divisions')
            openModal()
        } else if (button == 'exit') {
            home()
        }
