<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.categories')"></Head>
        
        <template #header>
            {{ $trnsl('content.categories') }}
        </template>
        ...
        <template #content>
            ...
            <DataTable
                ...
                :actions="actions"
                :dataset="categories"
                :title="trnsl('content.categories')"
                fieldSearch="name"
                @action="getAction"
            />
        </template>
...
<script setup>
    ...
    const headers = [
        {name: 'name', text: trnsl('content.category'), type: 'text', align: 'left'},
        {name: 'family.name', text: trnsl('content.family'), type: 'text', align: 'left'},
    ];
    ...
    const fields = [
        {name: 'family_id', type: 'select', text: trnsl('content.family'), style: 'sm:col-span-4', items:[], disabled: false}, 
        {name: 'name', type: 'text', text: trnsl('content.name'), style: 'sm:col-span-4', length: 255, disabled: false },
        {name: 'code', type: 'text', text: trnsl('content.code'), style: 'sm:col-span-4', length: 255, disabled: false },
        {name: 'description', type: 'text', text: trnsl('content.description'), style: 'sm:col-span-4', length: 255, disabled: false },
    ];
    ...
    const getAction = ( action ) => {
        if (action.name==='edit') {
            ...
            routePath.value = 'project.categories.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.categories')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'project.categories.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.categories')
            openModal()
        } else if (button == 'home') {
            home()
        }
