<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.families')"></Head>
        
        <template #header>
            {{ $trnsl('content.families') }}
        </template>
...
<script setup>
    ...
    const deleteRecord = (row) => {
        ...
        alert.fire({
            ...
            if(result.isConfirmed){
                form.delete(route('project.families.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const executeAction = ( params ) => {
        if (params[1]==='edit') {
            ...
            routePath.value = 'project.families.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.families')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'project.families.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.families')
            openModal()
        } else if (button == 'exit') {
            home()
        }
