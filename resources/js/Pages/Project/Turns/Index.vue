<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.turns')"></Head>
...
<script setup>
    ...
    const fields = [
                    ...
                    {name: 'nextday', type: 'check', text: trnsl('content.nextday'), style: 'sm:col-span-1', disabled: false },
    ...
    const deleteRecord = (row) => {
        ...
        alert.fire({
            ...
            if(result.isConfirmed){
                form.delete(route('project.turns.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const executeAction = ( params ) => {
        if (params[1]==='edit') {
            ...
            routePath.value = 'project.turns.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.turns')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'project.turns.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.turns')
            openModal()
        } else if (button == 'exit') {
            home()
        }
