<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.zones')"></Head>
...
<script setup>
    ...
    const deleteRecord = (row) => {
        ...
        dialog.fire().then((result) =>{
            if(result.isConfirmed){
                form.delete(route('project.zones.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const getAction = ( action ) => {
        if (action.name==='edit') {
            ...
            routePath.value = 'project.zones.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.zones')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'project.zones.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.zones')
