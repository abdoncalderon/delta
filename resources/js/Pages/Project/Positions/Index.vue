<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.positions')"></Head>
        
        <template #header>
            {{ $trnsl('content.positions') }}
        </template>
...
<script setup>
    ...
    const headers = [
        {name: 'name', text: trnsl('content.position'), type: 'text', align: 'left'},
        {name: 'function.name', text: trnsl('content.function'), type: 'text', align: 'left'},
    ];
    ...
    const fields = [
        ...
        {name: 'function_id', type: 'select', text: trnsl('content.function'), style: 'sm:col-span-4', items:[], disabled: false}, 
    ];
    ...
    const deleteRecord = (row) => {
        ...
        dialog.fire().then((result) =>{
            if(result.isConfirmed){
                form.delete(route('project.positions.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const getAction = ( action ) => {
        if (action.name==='edit') {
            ...
            routePath.value = 'project.positions.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.positions')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'project.positions.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.positions')
            openModal()
        } else if (button == 'home') {
            home()
        }
    ...
    onMounted(() => {
        fields[1].items = props.functions;
