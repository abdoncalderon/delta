<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.functions')"></Head>
        
        <template #header>
            {{ $trnsl('content.functions') }}
        </template>
...
<script setup>
    ...
    const deleteRecord = (row) => {
        ...
        alert.fire({
            ...
            if(result.isConfirmed){
                form.delete(route('project.functions.destroy', row.id),{
                    onSuccess: () => { 
    ...
    const executeAction = ( params ) => {
        if (params[1]==='edit') {
            ...
            routePath.value = 'project.functions.update'
            title.value = trnsl('content.edit') + ' ' + trnsl('content.functions')
    ...
    const clickIconToolBar = (button) => {
        if (button === 'new') {
            ...
            routePath.value = 'project.functions.store'
            title.value = trnsl('content.create') + ' ' + trnsl('content.functions')
            openModal()
        } else if (button == 'exit') {
            home()
        }
