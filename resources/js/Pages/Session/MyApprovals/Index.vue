<template>
    <AuthenticatedLayout>
        ...
        <Head :title="$trnsl('content.myApprovals')"/>
        ...
        <template #content>
            ...
            <DataTable
                ...
                :actions="actions"
                :dataset="myApprovals"
                :title="trnsl('content.myApprovals')"
                fieldSearch="name"
                @action="getAction"
            />
...
<script setup>
    ...
    const headers = [
        ...
        {name: 'project_user.user.person.fullName', text: trnsl('content.requestedBy'), type: 'text', align: 'left'},
    ];
    ...
    const refresh = () => {
        router.visit(route('myApprovals.index'))
    }
